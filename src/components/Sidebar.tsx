...
export default function Sidebar({ 
  ...
  return (
    <aside className={`transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-72'} bg-slate-900 text-white flex flex-col border-r border-slate-800 shrink-0 h-screen overflow-y-auto`}>
      {/* Brand Header */}
      {isCollapsed ? (
        ...
        <div className="p-6 border-b border-slate-800 flex items-center justify-between gap-2 group/brand">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="font-sans font-bold text-lg leading-tight tracking-tight text-white truncate" title={companyProfile?.name || 'LTL Logistics'}>
                {companyProfile?.shortName || 'LTL Logistics'}
              </h1>
              <span className="text-xs font-mono text-slate-400 block mt-1 truncate" title={companyProfile?.slogan}>
          ...
          <div className="flex items-center gap-1 shrink-0">
            {onEditCompany && (
              <button
                ...
                className="p-1.5 text-slate-500 hover:text-white hover:bg-slate-800 rounded transition duration-150 cursor-pointer"
            ...
            {onToggleCollapse && (
              <button
                onClick={onToggleCollapse}
                className="p-1.5 px-2 bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white rounded-lg transition cursor-pointer flex items-center justify-center shrink-0 border border-slate-700 hover:border-slate-650"
      ...
      {isCollapsed ? (
        <div className="p-2 mx-2.5 my-4 bg-slate-800/60 rounded-xl border border-slate-700/50 flex flex-col items-center gap-2" title={`${currentUser.fullName} (${currentUser.role})`}>
          ...
          <div className="w-full border-t border-slate-700/60 pt-2 flex flex-col items-center">
            <span className="text-[10px] text-emerald-400 font-mono font-bold">ON</span>
          </div>
        </div>
      ) : (
        <div className="p-4 mx-4 my-4 bg-slate-800/60 rounded-xl border border-slate-700/50">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm text-slate-400 font-medium">Đang đăng nhập:</p>
              <h4 className="text-base font-semibold truncate text-slate-200 mt-0.5">{currentUser.fullName}</h4>
              <div className="flex items-center gap-1.5 mt-1">
                <span className={`px-2 py-0.5 text-xs font-bold uppercase rounded tracking-wider ${
          ...
          <div className="mt-4 pt-3 border-t border-slate-700/60">
            <label className="block text-xs font-mono text-slate-500 uppercase mb-1">Mô phỏng vai trò khác:</label>
            <select 
              value={currentUser.id} 
              onChange={(e) => onChangeUser(e.target.value)}
              className="w-full bg-slate-900 border border-slate-700 rounded-md px-2 py-1.5 text-sm text-slate-300 focus:outline-none focus:ring-1 focus:ring-blue-500 cursor-pointer"
            >
              {allUsers.map(u => (
        ...
        {menuItems.map((item) => {
          ...
          if (isCollapsed) {
            return (
              <button
                ...
                className={`w-full flex items-center justify-center p-3.5 rounded-lg text-base font-medium transition-all duration-150 group relative ${
                  isActive 
                    ? 'bg-blue-600 text-white font-semibold' 
                    : 'text-slate-400 hover:bg-slate-800 hover:text-slate-100'
                }`}
              >
                <IconComponent className={`w-6 h-6 shrink-0 transition-colors ${
          ...
          return (
            <button
              key={item.id}
              onClick={() => setCurrentTab(item.id)}
              className={`w-full flex items-center justify-between px-4 py-3.5 rounded-lg text-base font-medium transition-all duration-150 group ${
                isActive 
                  ? 'bg-blue-600 text-white font-semibold' 
                  : 'text-slate-400 hover:bg-slate-800 hover:text-slate-100'
              }`}
